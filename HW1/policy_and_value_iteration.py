        ...
        for s in range(num_spaces):
            ...
            for a in range(num_actions):
                ...
                for s_ in range(num_spaces):
                    val += R[s][a][s_] + gamma * P[s][a][s_] * _V[s]
                if val > max_v:
                    max_v, max_a = val, a
            V[s], policy[s] = max_v, max_a
...
def policy_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    while first_iter or (policy - _policy).any():
        if first_iter:
            first_iter = False
        _policy = policy.copy()
        ...
        for i in range(max_iterations):
            ...
            for s in range(num_spaces):
                val = 0
                for s_ in range(num_spaces):
                    val += R[s][policy[s]][s_] + gamma * P[s][policy[s]][s_] * _V[s]
                V[s] = val
                delta = max(delta, abs(V[s] - _V[s]))
            if delta < eps:
                break
        ...
        for s in range(num_spaces):
            ...
            for a in range(num_actions):
                val = 0
                for s_ in range(num_spaces):
                    val += R[s][a][s_] + gamma * P[s][a][s_] * V[s]
                if val > max_v:
                    max_v, max_a = val, a
            policy[s] = max_a
